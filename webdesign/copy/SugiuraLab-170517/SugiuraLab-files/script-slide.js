...
var mouse = {
    ...
    windowWidth = windowHeight = windowWidthHalf = windowHeightHalf = 0,
    $window, $html, is_firstview = true;
$(function() {
    /*ブラウザの種類によって、スタイルの差し替え*/
    getBrows();
    ...
    /* 【重要】
    ウィンドウオブジェクトにonでプロパティを宣言しているため
    自動的に発火する関数として生成できている。
    つまりロードにはロードの、クリックにはクリックのプロパティがwindow自身に発火するよう設定されているので、簡単に値を取れる！
    */
    $window.on({
      /* ロード時に読み込む */
        load: function() {
...
var Page = {
    ...
    onBackground: function() {
        ...
        //initでカバーに入れたオブジェクトをeachで回す
        //カバーはスライドショー
        //each の引数は 第一がindex番号,第二がelements
...
/* スライドショーの右ボタンと左ボタンにそれぞれactionが付いている */
$(document).on('click', '.arrow-to-right', function(event) {
  /* 恐らく形式的にpreventDefaultされている　特に意味なし */
